...
void assertException(const char* exceptionClassName, std::function<void()> toCall) {
  ...
  //cout << " (" << info << ")";
  std::cout << " - Expected an exception to occur of type " << exceptionClassName << std::endl;
}

template <class P1, class P2>
void assertNearby(const char* info, P1 t1, P2 t2) {
  std::cout << ((t1 - t2) < 0.5 && (t2 - t1) < 0.5 ? "\033[32mPASSED\033[0m" : "\033[31mFAILED\033[0m");
  std::cout << " (" << info << ")";
  std::cout << " - Expected: " << t1 << ", " << "Got: " << t2 << std::endl;
}

template <class P1, class P2>
void assertEqual(const char* info, P1 t1, P2 t2) {
  std::cout << (t1 == t2 ? "\033[32mPASSED\033[0m" : "\033[31mFAILED\033[0m");
  std::cout << " (" << info << ")";
  std::cout << " - Expected: " << t1 << ", " << "Got: " << t2 << std::endl;
}
...
void title(const char* str) {
  std::cout << "\033[36;1m" << str << "\033[0m" << std::endl;
}
...
void assertSmaller(const char* info, P1 t1, P2 t2) {
  std::cout << (t2 < t1 ? "\033[32mPASSED\033[0m" : "\033[31mFAILED\033[0m");
  std::cout << " (" << info << ")";
  std::cout << " - Expected: " << t1 << ", " << "Got: " << t2 << std::endl;
}
