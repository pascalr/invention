...
Axis axisX, axisY, axisT, axisW, oups;
...
void parseMove(String cmd) {
  for (int i = 0; i < cmd.length(); i++) {
    int nbLength = numberLength(cmd.substring(i+1));
    Axis& axis = axisByLetter(cmd[i]);
    axis.previousStepTime = currentTime; // This is rather bad I think because it could cause the first step to be missed, but this is in order to refresh to previousStepTime
